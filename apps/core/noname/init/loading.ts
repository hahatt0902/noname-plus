		...
		switch (configName) {
			...
			default:
				for (const [itemName, item] of Object.entries(configItem)) {
					if (configName === "skill" && itemName[0] === "_" && !item.forceLoad && !lib.config.cards.includes(cardConfigName) && lib.config.mode !== "connect") {
						continue;
					}
		...
		switch (key) {
			...
			default:
				...
				for (let key2 in value) {
					...
					if (key === "skill" && key2[0] === "_" && !lib.config.characters.includes(name) && lib.config.mode !== "connect") {
						continue;
					}
