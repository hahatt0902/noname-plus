...
export class Create {
	...
	connectRooms(list) {
		...
		searchInput.className = "room-search-bar";
		searchInput.placeholder = "搜索房间（模式 / 房主名称）...";
		searchInput.style.webkitUserSelect = "text";
		searchInput.style.userSelect = "text";
		ui.roombase.contentContainer.insertBefore(searchInput, ui.roombase.contentContainer.firstChild);
		ui.roomSearchInput = searchInput;

		// 筛选按钮行
		var filterBar = document.createElement("div");
		filterBar.className = "room-filter-bar";
		var filterButtons = [
			{ key: "all", label: "全部" },
			{ key: "waiting", label: "等待中" },
			{ key: "gaming", label: "游戏中" },
		];
		var activeFilter = "all";
		var updateFilter = function (key) {
			activeFilter = key;
			filterBtns.forEach(function (btn) {
				if (btn._filterKey === key) {
					btn.classList.add("active");
				} else {
					btn.classList.remove("active");
				}
			});
			ui.filterRooms && ui.filterRooms();
		};
		var filterBtns = filterButtons.map(function (f) {
			var btn = document.createElement("div");
			btn.className = "room-filter-btn" + (f.key === "all" ? " active" : "");
			btn.textContent = f.label;
			btn._filterKey = f.key;
		...
		ui.roombase.contentContainer.insertBefore(filterBar, searchInput.nextSibling);

		// 筛选逻辑
		ui.filterRooms = function () {
			if (!ui.rooms) return;
			var query = searchInput.value.trim().toLowerCase();
			var emptyHint = ui.roombase.contentContainer.querySelector(".room-empty-hint");
			var visibleCount = 0;
			ui.rooms.forEach(function (player) {
				var visible = true;
				// 文字搜索（搜索内层 div 的文字内容）
				if (query) {
					var innerText = (player.firstChild ? player.firstChild.textContent : "").toLowerCase();
				...
				if (activeFilter === "waiting") {
					if (player.roomgaming || player.roomempty) {
						visible = false;
					}
				} else if (activeFilter === "gaming") {
					if (!player.roomgaming) {
						visible = false;
					}
				}
			...
			if (!emptyHint) {
				...
				emptyHint.className = "room-empty-hint";
				emptyHint.textContent = "暂无符合条件的房间";
				ui.roombase.contentContainer.appendChild(emptyHint);
			}
		...
		var addRoomItem = function (info, roomIndex) {
			var player = ui.roombase.add(
				'<div class="popup text pointerdiv room-item-new">空房间</div>'
			);
			...
			player.initRoom(info);
			return player;
		};

		for (var i = 0; i < list.length; i++) {
			var player = addRoomItem(list[i], i);
	...
	arena() {
		...
		ui.config2.style.display = "none"; // 仅保留联机功能，隐藏选项按针
		...
		if (!lib.config.touchscreen) {
			// lib.setPopped(ui.config2, ui.click.pauseconfig, 170); // 选项已禁用
		}
