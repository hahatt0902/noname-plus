	...
	element = {
		...
		ws: {
			onopen: function () {
				...
				// 创建/重置网络状态指示器
				if (!ui.networkIndicator) {
					var indicator = document.createElement("div");
					indicator.className = "network-indicator hidden";
					var signal = document.createElement("div");
					signal.className = "network-signal";
					for (var b = 0; b < 4; b++) {
						var bar = document.createElement("div");
						bar.className = "network-signal-bar";
						signal.appendChild(bar);
					}
					indicator.appendChild(signal);
					var pingText = document.createElement("div");
					pingText.className = "network-ping-text";
					pingText.textContent = "--ms";
					indicator.appendChild(pingText);
					ui.window.appendChild(indicator);
					ui.networkIndicator = indicator;
					ui.networkPingText = pingText;
				}
				ui.networkIndicator.classList.remove("hidden");
				_status.lastPingTime = null;
				_status.pingInterval = setInterval(function () {
					if (game.ws && game.ws.readyState === 1) {
						_status.lastPingTime = Date.now();
					}
				}, 5000);
			},
			onmessage: function (messageevent) {
				if (messageevent.data == "heartbeat") {
					// 计算延迟
					if (_status.lastPingTime != null) {
						var latency = Date.now() - _status.lastPingTime;
						_status.lastPingTime = null;
						if (ui.networkIndicator) {
							ui.networkPingText.textContent = latency + "ms";
							ui.networkIndicator.classList.remove("good", "medium", "poor");
							if (latency < 100) {
								ui.networkIndicator.classList.add("good");
							} else if (latency < 300) {
								ui.networkIndicator.classList.add("medium");
							} else {
								ui.networkIndicator.classList.add("poor");
							}
						}
					}
					this.send("heartbeat");
			...
			onclose: function () {
				...
				// 清理网络状态指示器
				if (_status.pingInterval) {
					clearInterval(_status.pingInterval);
					delete _status.pingInterval;
				}
				if (ui.networkIndicator) {
					ui.networkIndicator.classList.add("hidden");
				}
		...
		client: {
			...
			updaterooms: function (list, clients) {
				if (ui.rooms) {
					...
					if (false && !_status.requestReadClipboard && get.config("read_clipboard", "connect")) {
						const read = text => {
							try {
