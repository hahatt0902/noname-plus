...
export class Player extends HTMLDivElement {
	...
	*iterableGetKnownCards(other, filter) {
	...
	initRoom(info, info2) {
		this.serving = false;
		// 使用新式卡片布局渲染房间（仅当 firstChild 有 room-item-new 类时）
		var innerEl = this.firstChild;
		var isNewStyle = !!(innerEl && innerEl.classList && innerEl.classList.contains("room-item-new"));
		...
		if (!info || info == "server") {
			...
			if (isNewStyle) {
				innerEl.innerHTML = '<span style="color:rgba(255,255,255,0.5)">空房间</span>';
				innerEl.style.display = "";
			} else {
				this.firstChild.innerHTML = "空房间";
			}
			return this;
		}
		...
		if (!info.length) {
			...
			if (isNewStyle) {
				innerEl.innerHTML = '<span style="color:rgba(255,100,100,0.7)">异常房间</span>';
			} else {
				this.firstChild.innerHTML = "异常房间";
			}
			return this;
		}
		...
		var modeName = get.modetrans(config);
		var isFull = this.hp == this.maxHp && !config.gameStarted;
		var isGaming = config.gameStarted && (!config.observe || !config.observeReady);

		this.roomfull = isFull;
		this.roomgaming = isGaming;
		...
		if (isNewStyle) {
			// 新式卡片布局（操作内层 div）
			innerEl.innerHTML = "";
			innerEl.classList.toggle("room-gaming", !!config.gameStarted);
			innerEl.classList.toggle("room-full", isFull);

			// 模式标签
			var modeBadge = document.createElement("div");
			modeBadge.className = "room-item-mode-badge";
			modeBadge.textContent = modeName;
			innerEl.appendChild(modeBadge);

			// 中间信息列
			var infoCol = document.createElement("div");
			infoCol.className = "room-item-info";

			var ownerDiv = document.createElement("div");
			ownerDiv.className = "room-item-owner";
			ownerDiv.textContent = info[0].slice(0, 16) + " 的房间";
			infoCol.appendChild(ownerDiv);

			// 卡牌包信息
			if (config.mode != "guozhan" && !(config.mode == "doudizhu" && config.doudizhu_mode == "online") && config.cardPack && config.cardPack.length) {
				var packsDiv = document.createElement("div");
				packsDiv.className = "room-item-packs";
				var packNames = config.cardPack.map(function (p) {
					return get.translation(p + "_card_config").slice(0, 2);
				});
				packsDiv.textContent = "【" + packNames.join("+") + "】";
				infoCol.appendChild(packsDiv);
			}

			innerEl.appendChild(infoCol);

			// 状态标签
			var statusBadge = document.createElement("div");
			statusBadge.className = "room-item-status-badge " + (config.gameStarted ? "gaming" : "waiting");
			statusBadge.textContent = config.gameStarted ? "游戏中" : "等待中";
			innerEl.appendChild(statusBadge);

			// 人数标签
			var countBadge = document.createElement("div");
			countBadge.className = "room-item-count" + (isFull ? " full" : "");
			countBadge.textContent = this.hp + "/" + this.maxHp;
			innerEl.appendChild(countBadge);
		} else {
			// 原有文本布局（向后兼容）
			var str = modeName + " 模式　";
	...
	say(str) {
		...
		// 聊天面板未打开时，在聊天按钮上显示红色未读气泡
		if (this != game.me && ui.chatButton && (!_status.addChatEntry || !_status.addChatEntry._origin || !_status.addChatEntry._origin.parentNode)) {
			ui.chatButton.classList.add("has-unread");
			var badge = ui.chatButton.querySelector(".chat-unread-badge");
			var count = (parseInt(ui.chatButton.dataset.unread, 10) || 0) + 1;
			ui.chatButton.dataset.unread = count;
			if (!badge) {
				badge = document.createElement("div");
				badge.className = "chat-unread-badge";
				ui.chatButton.appendChild(badge);
			}
